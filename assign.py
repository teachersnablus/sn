...
if st.session_state['user_type'] == "school":
    ...
    if menu == "تعبئة وبحث":
        ...
        # محاولة جلب بيانات موجودة مسبقاً
        found_data = None
        if search_id:
        ...
        with t_m:
            if get_form_status('ثانوية') or get_form_status('توظيف'):
                ...
                with st.form("main_form", clear_on_submit=True):
                    ...
                    # إذا وجد بيانات في البحث، نضعها كقيم افتراضية
            ...
            else: st.warning("النماذج مغلقة من الإدارة")
                ...
                with st.form("corr_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("💾 حفظ طلب التصحيح"):
                        if c_id.strip() == "":
                            ...
                            conn.commit(); st.success("✅ تم حفظ طلب التصحيح"); st.rerun()
        ...
        st.write("📊 سجلات مدرستكم المكتملة:")
        if not df1.empty: st.dataframe(df1)
        if not df2.empty: st.dataframe(df2)
        
        st.divider()
        all_ids = list(set(df1['id_num'].tolist() + df2['id_num'].tolist()))
        target = st.selectbox("اختر رقم الهوية للتعديل الشامل أو الحذف:", [""] + all_ids)
        if target:
            is_main = target in df1['id_num'].values
            row = df1[df1['id_num']==target].iloc[0] if is_main else df2[df2['id_num']==target].iloc[0]
            with st.expander(f"🛠️ تعديل شامل لبيانات: {row[1]}"):
                un = st.text_input("الاسم", value=row[1])
                up = st.text_input("الجوال", value=row[5] if is_main else row[10])
                uc = st.text_input("المدينة", value=row[6])
                uv = st.text_input("القرية", value=row[7])
    ...
    if adm_menu == "صلاحيات النماذج":
        ...
        for i, f in enumerate(['ثانوية', 'توظيف', 'تصحيح']):
            with cols[i]:
                ...
                if st.button(f"تغيير الحالة لـ {f}"):
        ...
        def view_data(t_name, d_type, is_corr=False):
            ...
            sel = st.selectbox(f"مدرسة ({t_name}):", schools)
            f_df = df if sel == "الكل" else df[df['school_full_name'] == sel]
            st.dataframe(f_df)
            # خيار الحذف للمسؤول
            target_del = st.selectbox(f"حذف هوية من ({t_name}):", [""] + f_df['id_num'].tolist())
            if st.button(f"تأكيد حذف {target_del}"):
                c.execute(f"DELETE FROM {'correction_table' if is_corr else 'main_table'} WHERE id_num='{target_del}'")
                conn.commit(); st.rerun()
        
        with tab1: view_data("ثانوية عامة", "الثانوية العامة")
        with tab2: view_data("توظيف", "امتحان التوظيف")
        with tab3: view_data("تصحيح", "", True)
